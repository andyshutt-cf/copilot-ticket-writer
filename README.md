...
cp -r scripts /path/to/client-project/
...
5. Post the ticket to Jira using one of the methods below:
   - **CLI script:** save the Markdown to a file, then run `node scripts/create-jira-ticket.js --file ticket.md`
...
`scripts/create-jira-ticket.js` creates a Jira issue from a Markdown ticket file generated by the agent.
...
node scripts/create-jira-ticket.js --file ticket.md --project ACME

# Override the issue type (auto-detected if omitted)
node scripts/create-jira-ticket.js --file ticket.md --type Bug
...
3. Run `node scripts/create-jira-ticket.js --help` to verify the installation.
