...
**Steps**
1. Update `.github/agents/ticket-writer.agent.md` so the agent supports two explicit modes. Default mode remains the current immediate-draft flow. If the incoming request includes `--interactive`, the agent must switch into `Interactive Plan Mode`: scan codebase/docs first, ask 3-4 numbered clarification questions, give each question `A`, `B`, and `C Custom` options, pause for the user response, then draft the ticket only after answers arrive.
2. Revise `.github/ticket-writer-rules/ticket-format.instructions.md` to describe the dual-mode workflow clearly. Keep `Draft First, Refine After` as the default path, and add a parallel `Interactive Mode` path triggered by `--interactive`. Define the fallback rule inside interactive mode: if the user gives partial answers, draft with explicit assumptions after one clarification round rather than remaining blocked indefinitely. This step depends on step 1.
3. Align `ticket-writer-docs/knowledge/aacf-ticket-writer-requirements.md` with the dual-mode behaviour. The requirements document should state that immediate drafting is the default, but `--interactive` switches the agent into a pre-ticket interview flow with numbered A/B/C questions and a pause-for-response state. This step depends on steps 1-2.
...
2. Confirm the interactive path always requires 3-4 numbered questions with `A`, `B`, and `C Custom` options before ticket generation.
...
5. Confirm the fallback rule for partial answers is consistent everywhere.
